---
# the default layout is 'page'
icon: fas fa-info-circle
order: 4
---


## About Myself

Hello all! My name is Alexander Carter, and I am a passionate Cyber-Security student currently enrolled in Eastern Michigan University's Bachelors/Masters Combined Cybersecurity Program. 

Email: Acarte87@emich.edu
Github: https://github.com/aycftf

Specialties: 

-End user device security + hardware & software troubleshooting
-Network / operating-system hardening and provisioning
-Cloud (PaaS, SaaS, IaaS) automation, logging, monitoring and hardening 
-Compliance auditing and standards (ISO/IEC 27001, DFARS, NIST SP 800-53 / 171, FIPS, PCI DSS)
-IoT configuration
- Python, Shell (Bash), PowerShell, JS, HTML, XSS, SQLi/NOSQL, API’s. Majority of focus revolves around scripting automation.
- Over two years of hands-on experience in both personal and professional environments.

_I currently have my A+; I am currently aiming for my CPTS and CCNA certifications._

<span style="background: linear-gradient(90deg, #B7C240, #57C785); -webkit-background-clip: text; color: transparent; font-size: 60%"> Credly Badge: </span>

<div data-iframe-width="150" data-iframe-height="250" data-share-badge-id="a027feb7-fed4-4ac3-b749-1ef627ec0a70" data-share-badge-host="https://www.credly.com"></div><script type="text/javascript" async src="https://cdn.credly.com/assets/utilities/embed.js"></script>

Two personal fun facts about me are that I personally love a good hike with my black lab, Whiskey! Moreover, I mostly enjoy spending time with friends and family, exploring, and traveling!

![img-description](/assets/images/friends.jpeg)
<br><em>Me and my friends, in Holland Michigan!</em>



